...
public class CustomCameraView extends SurfaceView implements SurfaceHolder.Callback, PreviewCallback
{
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h)
    {
        if (this.surfaceHolder.getSurface() == null) {
            LogHelper.e(TAG, "surfaceChanged(): surfaceHolder is null, nothing to do.");
            return;
        }
    ...
    private void disconnectCamera()
    {
        ...
        try {
            stopThread = true;
            LogHelper.d(TAG, "Notify thread");
            synchronized (this) {
                this.notify();
            }
            LogHelper.d(TAG, "Wating for thread");
            if (thread != null)
                thread.join();
    ...
    private boolean initializeCamera()
    {
        synchronized (this) {
            if (this.cameraDevice != null) {
                ...
                LogHelper.i(TAG, "initializeCamera(): camera is already set, nothing to do");
                return true;
            }
            ...
            if (Build.VERSION.SDK_INT >= 9) {
                if (this.cameraId < 0) {
                    ...
                    for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
                        ...
                        if (camInfo.facing == cameraType) {
                            ...
                            try {
                                ...
                                LogHelper.e(TAG, "initializeCamera(): trying to open camera #" + i + " but it's locked", e);
                            }
                        }
                    }
                } else {
                    ...
                    try {
                        ...
                        LogHelper.e(TAG, "initializeCamera(): trying to re-open camera #" + this.cameraId + " but it's locked", e);
                    }
                }
            }
    ...
    private boolean setupCamera()
    {
        if (cameraDevice == null) {
            LogHelper.e(TAG, "setupCamera(): warning, camera is null");
            return false;
        }
        try {
    ...
    {
        LogHelper.d(TAG, "setCameraCallback()");
    ...
    private Size getBestSize(List<Size> sizes, int widthThreshold)
    {
        ...
        if (bestSize == null) {
            ...
            LogHelper.e(TAG, "determineBestSize(): can't find a good size. Setting to the very first...");
        }

        LogHelper.i(TAG, "determineBestSize(): bestSize is " + bestSize.width + "x" + bestSize.height);
        return bestSize;
    }
...
//            filter.stop();
    ...
    {
        if (cameraDevice == null) {
            LogHelper.e(TAG, "updateCameraDisplayOrientation(): warning, camera is null");
            return;
        }
    ...
    private class CameraWorker implements Runnable
    {
        public void run()
        {
            ...
            LogHelper.d(TAG, "Finish processing thread");
        }
    }
